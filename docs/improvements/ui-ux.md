...
**Adendo (2026-07, direção do usuário):** o app ganhou uma **tela inicial** (`TitleScreen`, rota `TitleRoute`, novo destino inicial) antes do menu de fases, no padrão do gênero: arte de fundo em tela cheia + botões Jogar / Modo Zen / ajustes / toggles de som. A arte é um único drawable **trocável** (`R.drawable.bg_title`) sem texto ou botões "martelados" — para uma nova season basta substituir o arquivo (`drawable-nodpi/bg_title.png|webp`, removendo o placeholder `drawable/bg_title.xml`); toda a UI é Compose sobreposta em posições fixas. Com isso o `safeDrawingPadding()` saiu do `NavHost` e passou a ser aplicado por destino (a tela inicial desenha edge-to-edge e trata os insets internamente). O fluxo de navegação passou a ser Título → Menu de fases → Jogo; o redesenho do menu de fases e da tela de jogo com a nova direção de arte (gerada via imagem + implementada em Compose) virá em seguida, tela a tela.
...
**Adendo (2026-07, direção do usuário — 2ª iteração, substitui a anterior):** o seletor de fases virou um **mapa-constelação**. O experimento anterior (arte de trilha `bg_phase_selector.png` com âncoras calibradas à mão) foi descartado: os nós flutuavam sem relação com o caminho pintado e cada arte nova exigiria recalibrar coordenadas. No conceito atual o `JourneyMap` desenha tudo proceduralmente em Compose sobre uma textura de madeira seamless em tile infinito (`R.drawable.bg_stage_map`, CC0): posições dos nós vêm de uma função senoidal dupla do índice (escala para **qualquer** quantidade de fases sem dados de layout por fase — adicionar fase no JSON do catálogo basta), linhas douradas com glow ligam as fases alcançadas, linhas pontilhadas seguem para as bloqueadas, e pontinhos dourados decorativos (seed determinística) completam o clima de constelação. Os nós (`StageNodeButton`) usam sprites PNG trocáveis por estado (CC0 FreeCandyButtons): `node_completed` / `node_current` / `node_locked` em `drawable-nodpi/`, com número, glow de seleção, pulso da próxima jogável (via `graphicsLayer`, sem recomposição contínua) e estrelas compostos por cima — a arte permanece substituível sem tocar em código. O mapa é arrastável (scroll vertical com fling) e **abre auto-centralizado na fase atual** (`highestUnlockedStage`), para jogadores avançados nunca precisarem rolar até ela. Escopo futuro: paleta/textura por região e troca de trilha musical ao rolar.
