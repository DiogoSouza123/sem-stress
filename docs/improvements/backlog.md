...
- [ ] **CQ-01** `P` — detekt + ktlint/formatting + lint baseline configurados localmente e no CI.

## Fase 1 — Rede de segurança (P0)

- [ ] **RR-19** `M` — Portar as 6 suítes de teste de engine do desktop para JUnit 5 (cascata, pontuação, validação, shuffle, board/seed) + helper `boardFrom(String)`. *Aceite:* cenários equivalentes ao desktop passando; `:app` (futuro `:core:engine`) ≥ 90% de cobertura na engine. ([code-quality](code-quality.md) §2.2)
...
- [ ] **RR-01** `G` — `GameViewModel` + `MenuViewModel` (StateFlow, `viewModelScope`, `SavedStateHandle`); controller eliminado. *Aceite:* rotação/process death preservam a partida; testes de CQ-02 passam. ([architecture](architecture.md) §2.3)
- [ ] **RR-02** `M` — Carregamento de fases/progresso assíncrono (IO dispatcher) + estado `Loading`; StrictMode em debug. *Aceite:* zero disk reads na main thread no fluxo de abertura.
...
- [ ] **RR-09** `M` — Código 100% inglês; strings de usuário em `strings.xml`. PR isolado, sem lógica.
