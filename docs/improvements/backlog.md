...
- [x] **UX-11** `M` — Acessibilidade: semântica no Canvas, modo símbolos, motion reduzido, alvos ≥ 48 dp, contraste AA. (escopo reduzido a uma fatia representativa: `SettingsRepository`/`SettingsViewModel` ganham `symbolModeEnabled`, consumido pelo `BoardCanvas` — `drawPiece` sempre desenha o glifo `fallbackPieceSymbol` em vez do sprite quando ativo, para quem não distingue cor/arte; `rememberReducedMotionEnabled` lê `Settings.Global.ANIMATOR_DURATION_SCALE` e desliga o shake de jogada invalida e o pulso de "poucos movimentos" quando o sistema pede motion reduzido; `Scoreboard` ganha um `contentDescription` único via `semantics(mergeDescendants = true)` resumindo pontos/meta/movimentos para TalkBack. Deferido para itens futuros: semantica por-celula do board em Canvas (virtual nodes), alvos de toque ≥48dp acima do tamanho visual da celula, auditoria formal de contraste WCAG AA, e — mais importante — o toggle de UI para modo simbolos/motion reduzido ainda não existe em nenhuma tela; as strings `toggle_symbol_mode_on/off` já foram adicionadas mas ficam sem uso ate a tela de configuracoes do UX-10 existir)
- [x] **UX-10 / UX-12** `M` — Configurações/pausa/créditos + microcopy. (`SettingsScreen` nova (rota `SettingsRoute`, acessivel pelo icone de engrenagem no `StageMenuScreen`) centraliza os toggles de musica/efeitos/modo-simbolos (agora com um controle de UI de verdade, resolvendo a lacuna deixada pelo UX-11) e traz secoes de creditos (atribuicao das trilhas/efeitos de freetouse.com) e privacidade (texto fixo, sem coleta de dados). `GameScreen`'s `ExitConfirmationDialog` virou `PauseDialog` (Continuar/Recomecar/Sair), disparado pelo botao de voltar e pelo gesto de sistema, reaproveitando `onReplayStage`/`onBackToMenu` ja existentes — antes so havia confirmacao de saida. Microcopy (UX-12): as duas mensagens do jogo que ainda eram texto cru no `GameViewModel` ("Sem movimentos disponiveis...", "Combo xN!") viraram chaves estaveis (`SHUFFLED_MESSAGE_KEY`/`COMBO_MESSAGE_KEY_PREFIX`) resolvidas para `strings.xml` por `resolveGameMessage` no `GameScreen`, com tom mais acolhedor ("Sem jogadas por ai? Embaralhamos os graos pra voce!"); `INVALID_MOVE_MESSAGE` virou `INVALID_MOVE_MESSAGE_KEY` (nao e mais texto exibido, so um sentinela de SFX/haptic, ver UX-05). Escopo: idioma alternativo, "restaurar progresso" e uma politica de privacidade real (fora do placeholder) ficam para itens futuros)
