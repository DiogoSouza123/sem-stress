...
| Modo debug do jogo | Painel de debug (apenas `debugImplementation`): escolher seed do board, pular fase, +N movimentos, ativar flags locais | Testar gameplay sem grind — essencial para balancear fases. **Feito em CQ-03**: `DebugMenuHost` (interface em `app/src/main`) tem implementação real só em `app/src/debug` (`RealDebugMenuHost`) e no-op em `app/src/release` (reaproveitada pelo build type `benchmark` via `sourceSets["benchmark"].java.srcDirs`); nenhum código/afordance de debug entra no APK release. Reshuffle com seed usa `Match3Board(seed=)`; "pular fase" força vitória (`GameAction.DebugForceWin`); flags usam `MutableFeatureFlags` (MZ-01) |
